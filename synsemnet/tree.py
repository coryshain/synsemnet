## This file is lifted and modified from the ModelBlocks repository (https://github.com/modelblocks/modelblocks-release)
...
class Tree:
    ...
    def collapse_unary(self):
        while len(self.ch) == 1 and len(self.ch[0].ch) > 0:
    ...
    def print_indented(self, indent_level=0, n_spaces=2, buffer=None):
        if buffer is None:
            buffer = sys.stdout
        if len(self.ch) > 0:
            buffer.write(' ' * (indent_level * n_spaces) + '(%s' % self.c)
            if len(self.ch) == 1 and len(self.ch[0].ch) == 0:
                buffer.write(' %s)' % self.ch[0].c)
            else:
                buffer.write('\n')
                for i, x in enumerate(self.ch):
                    x.print_indented(indent_level=indent_level+1, n_spaces=n_spaces)
                    if i < len(self.ch) - 1:
                        buffer.write('\n')
                    else:
                        buffer.write(')')
